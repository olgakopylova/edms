...
<div class="row" id="row_prot" {if hide}hidden{/if}>
    <div class="form-group col-md-12">
        <label for="id_prot" class="control-label small">Документ</label>
        <select name="id_prot" class="selectpicker form-control" id="id_prot" disabled>{#rows0}</select>
    </div>
</div>
<div class="row">
    <div class="form-group col-md-6 required">
        <label for="from" class="control-label small">Инициатор</label>
        <select name="from" class="selectpicker form-control" id="from" data-live-search="true" {if not_mult}disabled{/if}>{#rows1}</select>
    </div>
    <div class="form-group col-md-6 required">
        <label for="id_user" class="control-label small">Исполнители</label>
        {if !not_mult}<select name="id_user[]" id="id_user" class="selectpicker form-control" data-live-search="true" multiple>{#rows2}</select>{/if}
        {if not_mult}<select name="id_user[]" id="id_user" class="selectpicker form-control" data-live-search="true" multiple disabled>{#rows2}</select>{/if}
...
<div class="row">
    ...
    <div class="form-group col-md-6">
        ...
        <select name="id_resp" id="id_resp" class="selectpicker form-control" data-live-search="true" {if not_mult}disabled{/if}>{#rows4}</select>
    </div>
</div>
<div class="row">
    <div class="col-md-6"></div>
    <div class="form-group col-md-6">
        ...
        <select name="id_watcher[]" id="id_watcher" class="selectpicker form-control" data-live-search="true" multiple {if not_mult}disabled{/if}>{#rows3}</select>
    </div>
</div>
<div class="row">
...
<div class="row">
    <div class="form-group col-md-6 required">
        ...
        <input type="date" id="date_end" name="date_end" class="form-control {if date_end_err}is-invalid{/if}" value="{#date_end}" required {if disabled}disabled{/if}>
    </div>
...
<div class="row">
    <div class="form-group col-md-12">
        <input type="checkbox" class="form-check-input" id="is_tracked" name="is_tracked" value="0" {if checked}checked{/if} {if disabled}disabled{/if}>
        <label class="form-check-label small" for="is_tracked">Без отслеживания выполнения</label>
    </div>
</div>
